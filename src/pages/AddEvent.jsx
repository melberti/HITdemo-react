...
import { useVenues } from "../features/venue/useVenues";
...
function AddEvent({ event, onCloseModal }) {
  ...
  const venueFilter = filterParams.get("selectVenues") || "my";
  ...
  const handleVenueSelect = (valueFromModal) => {
    setVenueId(valueFromModal);

    toast("ATTENTION! Click the Update Event button below to save change", {
      duration: 6000,
      style: {
        color: "#b91c1c",
        fontWeight: "700",
      ...
      icon: <PiWarningDiamondBold size="75" className="text-red-700" />,
    });
  };
  ...
  let displayVenues;
  if (venueFilter === "my")
    displayVenues = venues.filter((venue) => venue.userId === user.id);
      ...
      <form onSubmit={handleSubmit(submitFunc)}>
        <Modal>
          <FormContainer>
            ...
            <FormButtonRow>
              <SubmitButton disabled={isAdding}>
